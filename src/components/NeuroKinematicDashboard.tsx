...
interface NeuroKinematicDashboardProps {
...
const NeuroKinematicDashboard = ({ useLiveBackend = false }: NeuroKinematicDashboardProps) => {
    ...
    const { data: backendData, isConnected } = useNeuroStream(
        '/ws/stream',
        useLiveBackend
    );
    ...
    useEffect(() => {
        ...
        const { sim_params, current_action } = backendData as any;
        if (sim_params) {
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            let theta: number, beta: number, alphaPower: number, betaPower: number;

            if (useLiveBackend && (backendData as any)?.sim_params) {
                theta = (backendData as any).sim_params.theta || 0.5;
                beta = (backendData as any).sim_params.beta || 0.5;
                alphaPower = (1 - theta) * 100;
                betaPower = beta * 100;
            } else {
