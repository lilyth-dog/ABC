...
안정적으로 동작하는지, 그리고 기존 문서의 성능/정확도 주장이 현재 코드와 일치하는지를
검증한다. 분석은 새로 추가한 \texttt{backend/empirical\_analysis.py}와 기존 테스트/검증
스크립트를 함께 사용했다.
...
프론트엔드 7개 테스트가 실패했다. 원인은 (1) \texttt{/api/behavior} 응답에서
...
(4) fake timer 기반 welcome animation 테스트의 비결정성이다. 해당 결함을 수정한 후
동일 명령을 재실행했다.
...
항목 & 통과 & 전체 & 비고 \\
\midrule
백엔드 pytest & 31 & 31 & coverage 53.62\% \\
...
중요한 실패 사례는 계획 시간 테스트다. 이벤트가 \texttt{inventory\_change(1000ms)},
\texttt{inventory\_change(2000ms)}, \texttt{block\_place(5000ms)}일 때 기존 평가는
첫 준비 행동부터 첫 건축까지의 4000ms를 기대한다. 그러나 현재 구현은 준비 행동들 사이의
span인 1000ms를 반환한다. 따라서 이는 단순 성능 문제가 아니라 ``계획 시간''의 조작적 정의가
문서/평가와 구현 사이에서 불일치하는 반례다.
...
\begin{itemize}
  ...
  \item 성능은 1000 이벤트 p95 8.148ms, 5000 이벤트 p95 150.213ms로, 일반적인 수백~천 단위 세션에는 충분해 보인다.
  \item 계획 시간 정의 불일치는 남아 있다. 이 지표를 논문/발표에서 사용하려면 먼저 정의를 고정해야 한다.
  \item 실제 데이터 표본이 매우 작고, 생체신호 대용량 데이터가 없어 성격 추론 타당도나 사용자 수준 외적 타당도는 아직 입증되지 않았다.
...
그러나 ``성격 추론이 실제 행동 특성을 정확히 반영한다''는 강한 주장은 아직 데이터와 평가 설계가
부족하다. 이번 실증분석에서 확실히 말할 수 있는 결론은 다음이다:
(1) 수정 후 구현은 백엔드/프론트엔드 테스트를 모두 통과한다,
(2) 공개 샘플과 무작위 단일 변형 입력에서 파이프라인은 안정적으로 실행된다,
(3) 계획 시간 지표는 평가 정의와 구현 정의가 충돌하므로 후속 실험 전에 반드시 정리해야 한다.
