...
class InterpolatedSimulationDB:
    ...
    def _compute_pattern(self, X, Y, R, theta: float, beta: float) -> np.ndarray:
        ...
        - 더 정교한 물리 모델링 (Landau-Lifshitz-Gilbert 방정식 근사)
        
        theta: Relaxation parameter (0=alert, 1=relaxed)
        beta: Excitation parameter (0=calm, 1=excited)
        """
        # Physical parameters (더 정확한 물리 상수)
        alpha = 0.01 + 0.04 * theta  # Gilbert damping (0.01-0.05)
        freq = 5 + 15 * beta          # Excitation frequency (5-20 GHz)
        amplitude = 0.2 + 0.8 * beta  # Wave amplitude
        exchange_coupling = 13e-12   # Exchange constant (J/m) - Permalloy
        
        # 고급 패턴 구성 요소
        
        # 1. Central vortex (ground state, stronger at high theta)
        # Skyrmion-like structure
        vortex_angle = np.arctan2(Y, X)
        vortex_core = np.exp(-R**2 / 4) * theta
        vortex = np.sin(vortex_angle) * (1 - np.exp(-R)) * vortex_core
        
        # 2. Spin waves with dispersion relation
        # k = 2π/λ, ω = 2πf, v = ω/k
        k_vector = 0.5 * freq / (2 * np.pi)  # Wave number
        spin_wave = np.sin(k_vector * R - 2 * np.pi * freq * 0.1) * np.exp(-alpha * R * 2) * amplitude
        
        # 3. Domain walls with Bloch/Néel structure
        # Domain wall width: δ = sqrt(A/K), A=exchange, K=anisotropy
        domain_wall_width = 50e-9  # 50nm typical
        domain_wall = np.tanh(X / domain_wall_width * (1 + beta)) * np.exp(-alpha * 5)
        
        # 4. Interference patterns (multiple wave sources at high beta)
        if beta > 0.5:
        ...
        # 5. Spin precession (Larmor frequency)
        # ω_L = γ * B_eff, γ = gyromagnetic ratio
        larmor_freq = 28e9 * beta  # 28 GHz/T for electron
        precession = np.sin(larmor_freq * 0.1) * np.exp(-alpha * R * 0.3) * 0.1
        
        # 6. Thermal fluctuations (theta-dependent)
        thermal_noise = np.random.normal(0, 0.05 * (1 - theta), X.shape)
        
        # Combine based on parameter weights (물리적으로 더 정확한 가중치)
        pattern = (
            vortex * theta * 0.25 +           # Ground state
            spin_wave * beta * 0.35 +          # Excited states
            domain_wall * (1 - abs(theta - beta)) * 0.15 +  # Intermediate
            interference * 0.15 +              # High excitation
            precession * beta * 0.05 +         # Precession
            thermal_noise * 0.05                # Thermal
        )
        
        # Apply damping envelope (exponential decay)
